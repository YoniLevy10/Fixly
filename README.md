...
רשת ביצוע לתחזוקה ובעלי מקצוע בישראל — עברית קודם.

**לא** marketplace פתוח ו**לא** “Uber לבעלי מקצוע”.  
ביקוש אמיתי מ־**Bamakor (= BINO)** ומלקוחות עסקיים → התאמה לפי תחום, אזור, זמינות, מחיר וביצועים אובייקטיביים → מעקב עד סיום.  
צרכנים פרטיים נפתחים רק אחרי צפיפות מספקת באזור.

עקרונות מחייבים: [`docs/DIFFERENTIATION.md`](./docs/DIFFERENTIATION.md).

**סטטוס:** מוצר MVP מוכן בקוד. ההשקה תלויה בתשתית חיצונית (Supabase / Stripe / OAuth), בנפח Bamakor, ובגיוס ידני של בעלי מקצוע בעיר פיילוט — לא בפיתוח גנרי נוסף.
...
- התאמה לפי תחום / אזור / זמינות / מחיר / `performance_score`
- Partner API ל-Bamakor (`/api/v1/jobs` + webhooks + escalation metadata)
- שער צפיפות לצרכנים פרטיים (`launch_regions`)
- דשבורד לקוח / Pro, צ'אט על בקשה, מעקב חי
- מונטיזציה: קרדיטי לידים, מנוי Pro (₪149/חודש), עמלה על עסקה
- WhatsApp deep links, הפניות, אימות Pro, פאנל אדמין
