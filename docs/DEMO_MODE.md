...
- באנר כתום בראש האפליקציה עם:
  - החלפת תפקיד **לקוח ↔ בעל מקצוע**
  - קישור **התחל הזמנה מלאה**
  - כפתור **סיור 90 שניות** (אוטומטי)
...
1. **לקוח / בעל מקצוע** — החלפת תפקיד מיידית (בלי התחברות)
2. **התחל הזמנה מלאה** — יצירה ידנית מול Pro `#1`
3. **סיור 90 שניות** — יוצר בקשה, מאשר כ־Pro, מציג מפה חיה, משלים ומחזיר ללקוח לביקורת

כיבוי הדגל (`NEXT_PUBLIC_FF_DEMO_DATA=false`) מסתיר את כל ה־UI הזה ומחזיר את האפליקציה לנתוני Supabase בלבד.
...
| 3 | בבאנר לחצו **בעל מקצוע** (או `/pro/dashboard` → כניסת דמו) |
| 4 | אשרו את הבקשה → **בדרך** → **התחלת עבודה** → **סיום** |
| 5 | בבאנר לחצו **לקוח** — מפה חיה + ETA בזמן `בדרך` |
